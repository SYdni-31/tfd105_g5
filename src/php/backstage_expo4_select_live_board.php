<?php
    ...
    $sql2 = "set @a= concat('select ID, CONTENT, STATUS, LIVE_LIST_ID, COMPANY_ID, GUEST_ID, AGENDA_ID, NAME from view_live_board where ID like' , '\"%', :search_word, '%\"','or CONTENT like', '\"%', :search_word, '%\"', 'or STATUS like', '\"%', :search_word, '%\"', 'or LIVE_LIST_ID like', '\"%', :search_word, '%\"', 'or COMPANY_ID like', '\"%', :search_word, '%\"', 'or GUEST_ID like', '\"%', :search_word, '%\"', 'or GUEST_ID like', '\"%', :search_word, '%\"', 'or AGENDA_ID like', '\"%', :search_word, '%\"', 'or NAME like', '\"%', :search_word, '%\"', ' limit',' ', (:inpage-1)*:perpage, ', ', :perpage)";
