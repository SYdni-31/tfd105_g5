<?php
    ...
    $sql2= "set @a= concat('select * from view_agenda_speach where ID like' , '\"%', :search_word, '%\"','or NAME like', '\"%', :search_word, '%\"', 'or START_TIME like', '\"%', :search_word, '%\"', 'or END_TIME like', '\"%', :search_word, '%\"', 'or DATE like', '\"%', :search_word, '%\"', 'or THEME like', '\"%', :search_word, '%\"', ' order by DATE limit',' ', (:inpage-1)*:perpage, ', ', :perpage)";
