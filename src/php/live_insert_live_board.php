<?php
    ...
    // php的欄位全部都大寫
    $sql = "INSERT INTO LIVE_BOARD(CONTENT, STATUS, COMPANY_ID, GUEST_ID, AGENDA_ID) VALUES (:CONTENT, :STATUS, :COMPANY_ID, :GUEST_ID, (SELECT ID FROM AGENDA where date = CURDATE() and HOUR(START_TIME) = HOUR(now()) and HOUR(END_TIME) >= HOUR(now()) and STATUS !='D' order by ID desc limit 1))";
